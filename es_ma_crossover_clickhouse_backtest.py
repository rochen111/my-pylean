#!/usr/bin/env python3
"""Run an ES daily 50/200 MA crossover backtest from ClickHouse data.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ES MA crossover backtest from ClickHouse")
...
def build_equity_curve(df: pd.DataFrame, cfg: BacktestConfig) -> pd.DataFrame:
    ...
    data["fast_ma"] = data["close"].rolling(cfg.fast).mean()
    data["slow_ma"] = data["close"].rolling(cfg.slow).mean()
...
def main() -> int:
    ...
    print("== LEAN-style Backtest Rundown ==")
